...
if (isset($_POST["add"])) {
    if(isset($_SESSION["shopping_cart"])) {     
        $item_array_id = array_column($_SESSION["shopping_cart"], "item_id");  //trả về giá trị item_id của array 
        $item_array_quantity = array_column($_SESSION["shopping_cart"], "quantity");
        $count = count($_SESSION["shopping_cart"]);
        if(!in_array($_GET["id"], $item_array_id)) {  //tìm giá trị id từ get trong array $item_array_id
              //$count = count($_SESSION["shopping_cart"]);  
              $item_array = array(  
                   'item_id'               =>     $_GET["id"],  
            ...
            <?php while($row = mysqli_fetch_assoc($result)) : ?>
                <figure>
                    <form method="post" action="product.php?action=add&id=<?php echo $row["id"]; ?>">
                        ...
                        <h4 class="text-danger"><?php echo $row["price"]; ?>.000 VND</h4>
    ...
    <div style="width:700px; align:center" class="container table-responsive" >
        ...
        <table class="table table-bordered">  
            ...
            <tr>  
                ...
                <td><?php echo $values["item_price"]; ?>.000 VND</td>  
                <td><?php echo number_format($values["item_quantity"] * $values["item_price"], 2); ?>0 VND</td>  
